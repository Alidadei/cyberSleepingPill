...
2. **真正的天花板是 Supabase 免费档的每月下行流量（egress，5GB/月，2026-09-21 官方定价页核实）**，而不是"同时多少人"。
...
**额度细节（2026-09-21 于 supabase.com/pricing 核实）**：

- 免费档 **5GB/月下行流量（egress）**，另有 5GB「缓存命中 egress」额度——但本站 PostgREST 响应实测 `cf-cache-status: DYNAMIC`（不缓存），缓存份额用不上，**记账一律按 5GB/月**。
- 超额去处：Pro 档 $25/月 = 250GB（50 倍）；零预算路线是做下面的省流量改造（降一个数量级）。
- 两个不占此额度的流量：GitHub Pages 的页面下载走 GitHub 自己的配额（100GB/月软上限）；`/data` 页 30 秒自刷被密钥门禁挡住，公众烧不到。
- 附带确认：免费项目 1 周无活动会被暂停——本站每日死链巡检（Actions 查库）天然保活，此坑不存在。

主站每次打开 = 榜单 + 举报表全量拉取，流量随数据量线性涨（人数按人均每月来访 10 次折算）：

| 云端条目数 | 每次访问 API 流量（估） | 5GB ≈ 每月访问次数 | ≈ 每月支持人数 |
|---|---|---|---|
| 现在（个位数，实测 0.4KB/次） | <1KB | ~1200 万次 | 等于无限 |
| 50 条（冷启动目标） | ~20KB | ~25 万次 | ~2.5 万人 |
| 200 条（预警线） | ~80KB | ~6 万次 | ~6 千人 |
| 300 条 | ~120KB | ~4 万次 | ~4 千人 |
| 1000 条（拉取上限） | ~400KB | ~1.2 万次 | ~1 千人 |

**能用多久（三场景）**：现在（哪怕每天 100 人次打开）每月约 1MB，等于用不完；50 条 + 每天 100 人次 ≈ 60MB/月，够约 7 年；300 条 + 每天 500 人次 ≈ 1.8GB/月，**不到 3 个月烧完**——预警线定在 200–300 条的原因就在这。
...
前端显示效果不受影响（卡片/排序/功能全不动），唯二要细心处：搜索与「全部」排行需改**按需云端查询**（PostgREST `ilike` 够用，不能偷懒只搜已加载部分——会漏结果）；类型星图节点计数改用只拉 `type` 列的廉价请求保持精确。
